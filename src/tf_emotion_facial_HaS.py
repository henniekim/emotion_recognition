...
for i in range(epoch):
    ...
    for j in range(len(X_train) // batch_size):
        ...
        for num in range(0, batch_size):
            key = np.random.randint(4, size=1)  # 50 %
            if key == 0 : # eyes
                X_batch[num, :] = fill_area_with(X_batch[num,:], 10, 14, 10, 7, global_mean)
                X_batch[num, :] = fill_area_with(X_batch[num,:], 27, 14, 10, 7, global_mean)

            elif key == 1: # lip
                X_batch[num, :] = fill_area_with(X_batch[num,:], 14, 35, 19, 9, global_mean)

            elif key == 2: # nose
                X_batch[num, :] = fill_area_with(X_batch[num,:], 14, 26, 19, 5, global_mean)
